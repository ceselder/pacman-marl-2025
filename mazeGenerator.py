...
class Maze:
  ...
  def to_map(self):
    ...
    for row in range(self.r):
      for col in range(self.c-1, -1, -1):
        self.grid[row].append(self.grid[row][col])
    self.c *= 2
  ...
  def add_wall(self, i, gaps=1, vert=True):
    ...
    if vert:
      ...
      if not self.root.c-1 in slots and max(slots) + 1 < len(self.root.grid):
      ...
      if not 0 in slots:
        ...
        if len(slots) <= gaps: return 0
      if not self.root.r-1 in slots and max(slots) + 1 < len(self.root.grid[0]):
...
def generateMaze(seed = None):
  ...
  random.seed(seed)
  maze = Maze(18,9)
  gapfactor = random.gauss(0.7,0.2)
  skip = make_with_prison(maze, depth=0, gaps=5, vert=True, min_width=0, gapfactor=gapfactor)
  maze.to_map()
